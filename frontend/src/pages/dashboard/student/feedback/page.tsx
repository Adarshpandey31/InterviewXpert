...
// Mock feedback data
import { interviewFeedbacks } from "@/data/studentMockData"
import Footer from "@/components/footer"

export function FeedbackReportPage() {
  const { interviewId } = useParams();
  const feedbackData = useMemo(() => {
    return interviewFeedbacks.filter((feedback) => feedback.interviewId.toString() === interviewId)[0]
  }, [interviewId])
            ...
            <Card>
              ...
              <CardFooter className="flex flex-col items-start gap-2">
                <Button variant="outline" className="w-full" asChild>
                  <Link to={`/interview/${feedbackData.interview.id}/recording`}>View Recording</Link>
                </Button>
